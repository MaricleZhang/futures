...
class DeepLearningStrategy(BaseStrategy):
    ...
    def __init__(self, trader):
        ...
        self.prediction_length = 10  # 预测序列长度（未来的K线数量）
    ...
    def initialize_model(self):
        ...
        try:
            ...
            # 输出层 - 确保输出维度与预测长度一致
            model.add(Dense(self.prediction_length, activation='linear'))  # Changed to linear activation
    ...
    def prepare_features(self, klines):
        ...
        try:
            ...
            for i in range(len(df) - self.sequence_length - self.prediction_length + 1):
                ...
                # 确保y_sequence的长度为self.prediction_length
                if len(y_sequence) == self.prediction_length:
                    X.append(X_sequence)
                    y.append(y_sequence)
    ...
    def train_model(self, klines):
        ...
        try:
            ...
            # 验证输出维度是否匹配
            if y.shape[1] != self.prediction_length:
                self.logger.error(f"目标数据维度不匹配: 期望 {self.prediction_length}, 实际 {y.shape[1]}")
                return False
