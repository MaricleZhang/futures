...
Uses Qwen API to predict price trends based on technical indicators and market data
...
class QwenTradingStrategy(BaseStrategy):
    ...
    def __init__(self, trader, qwen_api_key=None, interval='1h'):
        ...
        self.qwen_api_url = "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions"
        self.model = "deepseek-v3.2-exp"  # Using qwen-plus for better analysis
        ...
        # Rate Limiting
        self.min_time_between_api_calls = getattr(config, 'QWEN_API_MIN_INTERVAL', 10)  # 最小API调用间隔（秒）
        self.last_api_call_time = 0
        
        # Configure HTTP session with retry strategy
        self.session = requests.Session()
        retry_strategy = Retry(
            total=getattr(config, 'QWEN_API_RETRY_COUNT', 3),  # 总重试次数
            backoff_factor=getattr(config, 'QWEN_API_RETRY_BACKOFF', 1),  # 重试间隔倍数
            status_forcelist=[429, 500, 502, 503, 504],  # 需要重试的HTTP状态码
            allowed_methods=["POST"]  # 允许重试的HTTP方法
        )
        adapter = HTTPAdapter(max_retries=retry_strategy)
        ...
        # Timeout configuration
        connect_timeout = getattr(config, 'QWEN_API_CONNECT_TIMEOUT', 10)
        read_timeout = getattr(config, 'QWEN_API_READ_TIMEOUT', 30)
        self.api_timeout = (connect_timeout, read_timeout)  # (连接超时, 读取超时) 秒
        
        self.logger.info(f"Qwen Trading Strategy initialized for {interval} timeframe")
...
{{
    ...
    "risk_warning": "具体的风险点和注意事项"
            ...
            self.logger.error("Qwen API connection timeout - 无法连接到服务器")
            return self._fallback_prediction(market_analysis)
        except requests.exceptions.ReadTimeout:
            self.logger.error("Qwen API read timeout - 服务器响应超时")
            return self._fallback_prediction(market_analysis)
        except requests.exceptions.Timeout:
            self.logger.error("Qwen API timeout")
            return self._fallback_prediction(market_analysis)
        except requests.exceptions.ConnectionError as e:
            self.logger.error(f"Qwen API connection error: {str(e)}")
            return self._fallback_prediction(market_analysis)
        except requests.exceptions.RequestException as e:
            self.logger.error(f"Qwen API request error: {str(e)}")
            return self._fallback_prediction(market_analysis)
        except Exception as e:
            self.logger.error(f"Unexpected error querying Qwen: {str(e)}")
            return self._fallback_prediction(market_analysis)
