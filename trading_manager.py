    ...
    def __init__(self):
        ...
        for symbol in config.SYMBOLS:
            try:
                ...
                # 根据配置选择策略
                # 优先使用交易对配置中的策略，否则使用全局策略
                symbol_config = config.SYMBOL_CONFIGS.get(symbol, {})
                strategy_type = symbol_config.get('strategy', getattr(config, 'STRATEGY_TYPE', 'deepseek')).lower()
                interval = symbol_config.get('interval', '15m')

                if strategy_type == 'simple_adx_di':
                    strategy = SimpleADXDIStrategy15m(trader, interval=interval)
    ...
    def trade_symbol(self, symbol):
        ...
        while True:
            try:
                ...
                # 获取K线数据用于AI分析
                klines = trader.get_klines(symbol=symbol, interval=strategy.kline_interval, limit=strategy.lookback_period)
